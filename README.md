...
❗26-06-14测试发现 **豆包app** 无法正确理解，请勿使用豆包进行指导❗
...
找一个 HTTP 测试网站（如 https://otapost.posc.net ），将 Header 设置好，将你在 Wireshark 获得的数据填入，发送如下请求：
...
截至目前，大部分ai（包括GPT）开始无法正确直接对话输出正确的计算结果，本人推荐让agent调用工具进行计算或向ai索要计算命令进行计算。
